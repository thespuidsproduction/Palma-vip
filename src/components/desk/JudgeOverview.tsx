...
import { Card, SectionHead, Tag, Dot, Glyph, Empty, Action, Notice } from './surface';
...
/* ───────────────────────────────────────────────────────────────────────────
   ...
   A judge arrives with one question — what is left — so the dial answers it
   before anything else. The four counts sit under it, the categories are
   meters, and the deadline is held in the corner where it can be glanced at
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        {stats.map((stat, i) => (
          <Card key={stat.label} index={i + 1} className="flex flex-col gap-4 p-4">
      ...
      <div className="grid gap-6 xl:grid-cols-12">
        ...
        <aside className="flex min-w-0 flex-col gap-4 xl:col-span-4">
          <Card elevation="raised" className="p-5">
            ...
            <p className="font-display text-xl leading-tight text-[color:var(--text)]">
            ...
            {season.closesAt ? (
              <p className="mt-1.5 text-[0.8125rem] text-[color:var(--text-quiet)]">
                {season.closesAt}
              </p>
            ) : null}

            {isChair ? (
              <p className="mt-4 border-t border-[color:var(--line)] pt-4 text-[0.75rem] leading-relaxed text-[color:var(--text-quiet)]">
          ...
          <Card reveal className="overflow-hidden">
            <div className="flex items-center gap-2.5 p-4">
              <Glyph icon={Bell} size="sm" />
              <h3 className="font-display text-[0.9375rem] text-[color:var(--text)]">Notices</h3>
              {notifications.length > 0 ? (
                <Tag className="ml-auto">{notifications.length}</Tag>
              ) : null}
            </div>
            <div className="h-px bg-[color:var(--line)]" />
            {notifications.length === 0 ? (
              <p className="p-4 text-[0.8125rem] leading-relaxed text-[color:var(--text-quiet)]">
              ...
              <ul className="flex flex-col">
                {notifications.map((notice) => (
                  <li
                    key={notice.id}
                    className="flex flex-col gap-1 border-b border-[color:var(--line)] p-4 last:border-none"
                  >
                    <span className="flex items-baseline justify-between gap-3">
                      <span className="font-display text-[0.9375rem] text-[color:var(--text)]">
                        {notice.subject}
                      </span>
                      <span className="label shrink-0 text-[10px]">{notice.when}</span>
                    </span>
                    <span className="text-[0.8125rem] leading-relaxed text-[color:var(--text-soft)]">
                      {notice.body}
                    </span>
                    {notice.href ? (
                      <Link
                        href={notice.href}
                        className="mt-1 inline-flex items-center gap-1 text-[0.8125rem] text-[color:var(--accent)] underline-offset-4 hover:underline"
                      >
          ...
          <Panel icon={Scale} title="How PALMA treats your work" reveal>
